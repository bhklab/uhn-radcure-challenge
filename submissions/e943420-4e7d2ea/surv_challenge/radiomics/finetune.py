...
def main(hparams):
    ...
    checkpoint_path = os.path.join(logger.experiment.get_logdir(),
                                   "checkpoints",
                                   "cnn_{epoch:02d}-{val_loss:.2e}-{roc_auc:.2f}")
    checkpoint_callback = ModelCheckpoint(filepath=checkpoint_path,
                                          save_top_k=5,
                                          monitor="roc_auc",
                                          mode="max")

    model = Challenger.load_from_checkpoint(checkpoint_path="ENTER_PATH_HERE")
...
if __name__ == "__main__":
    ...
    parser = ResumeModel.add_model_specific_args(parser)
